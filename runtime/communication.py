...
class CommunicationHandler(object):
    ...
    def initialize(self, receive_ranks, send_ranks,
        ...
        print("before setup queues")
        self.setup_queues()
        print("before messaging schedule")
        self.setup_messaging_schedule()
        print("before create_process_groups")
        self.create_process_groups()
        print("after create_process_groups")
    ...
    def create_process_groups(self):
        ...
        print("self rank in comm: ", self.rank, "process_groups: ", self.process_groups, "connection_list: ", self.connection_list)
    ...
    def recv_helper_thread_args(self, tensor_name, index, dtype,
        ...
        print("recv helper sub_process_group: ", sub_process_group)
        if backward:
    ...
    def send_helper_thread_args(self, tensor_name, index,
        ...
        print("send helper sub_process_group: ", sub_process_group)
        if backward:
...
def _recv(tensor_name, src_rank, tensor_shape=None, dtype=torch.float32,
    ...
    if sub_process_group is not None:
        ...
        received_tensor_shape = torch.zeros(len(tensor_shape), dtype=torch.int).cuda()
